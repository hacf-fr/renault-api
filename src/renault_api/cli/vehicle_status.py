...
async def update_battery_status(
    kamereon: Kamereon, account: str, vin: str, status_table: Dict[str, str]
) -> None:
    ...
    try:
        ...
        attributes: Dict[str, Any] = response.data.raw_data["attributes"]
        ...
        if "chargingInstantaneousPower" in attributes:
            status_table[
                "Charge rate"
            ] = f"{attributes['chargingInstantaneousPower']:.2f} kW"
        if "chargingRemainingTime" in attributes:
            status_table[
                "Time remaining"
            ] = f"{attributes['chargingRemainingTime']} min"
...
async def update_hvac_status(
    ...
    try:
        ...
        if "externalTemperature" in attributes:
            ...
            ] = f"{attributes['externalTemperature']} °C"
